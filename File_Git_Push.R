...
#create function for preparing the paste
testFUN <- function(i){
...
for(j in effect_sizes){
  ...
  for (i in temporary_data$variable) {
    temporary_filename <- paste("demo_plots/jitterplot",
                                paste(str_replace(i,"[.]","_")),
                                "d", 
                                substr(j, 3, 3), # sonst wird alles nach dem Puntk als Dateiendung verstanden
                                ".png",
                                sep = "_")
  }
